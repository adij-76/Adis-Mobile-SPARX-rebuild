...
| 10b | `db/ai-context.sql` | **Sparxy personalization**: `mobile_ai_context(auth_uid)` SECURITY DEFINER RPC → the whole per-user picture as one JSON (identity, focus/problems, latest assessments + full scored history, recent check-ins, 7-day activity rollup from `mobile_xp_events`, gamification, the user's recent posts, safety flags). **service_role only** (revoked from anon/authenticated) — server-side n8n use. Read-only. | ✅ |
